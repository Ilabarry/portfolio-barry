...
const Navbar = ({ darkMode, toggleDarkMode }) => {
  ...
  return (
    ...
    >
      {/* Container principal avec overflow control */}
      <div className="w-full max-w-[100vw] overflow-hidden">
        <nav className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          
          {/* Ligne principale - Optimisée pour tous les écrans */}
          <div className="flex items-center justify-between py-2 sm:py-3 md:py-4 min-h-[56px] sm:min-h-[60px]">
            ...
            <div className="flex items-center flex-shrink-0">
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  {/* Badges compétences - Responsive */}
                  <div className="flex items-center gap-1 sm:gap-2 text-xs text-gray-600 dark:text-gray-400 font-medium mt-0.5">
            ...
            <div className="hidden lg:flex items-center space-x-0 xl:space-x-1 flex-shrink-0">
              {navItems.map((item, index) => {
                ...
                return (
                  <a
                    key={item.href}
                    href={item.href}
                    className={`relative px-3 py-2 xl:px-4 xl:py-2.5 text-sm font-semibold transition-all duration-300 rounded-lg xl:rounded-xl mx-0.5 xl:mx-1 group ${
                      isActive 
            ...
            <div className="hidden md:flex items-center space-x-2 lg:space-x-3 flex-shrink-0">
              {/* Réseaux sociaux */}
              <div className="flex items-center space-x-1 bg-gray-100 dark:bg-gray-800/50 rounded-xl lg:rounded-2xl p-1 border border-gray-200 dark:border-gray-700">
                {professionalLinks.map((item, index) => (
                  <a
                    ...
                    className="p-1.5 sm:p-2 rounded-lg lg:rounded-xl border border-transparent bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 transition-all duration-300 hover:scale-105 hover:shadow-md hover:bg-blue-600 hover:text-white"
              ...
              {/* Séparateur - Visible à partir de lg */}
              <div className="hidden lg:block w-px h-6 bg-gradient-to-b from-gray-300 to-gray-100 dark:from-gray-700 dark:to-gray-600"></div>

              {/* Bouton thème */}
              <button 
                onClick={toggleDarkMode}
                className="p-1.5 sm:p-2 rounded-lg lg:rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 hover:shadow-md transition-all duration-300 hover:scale-105 group"
            ...
            <div className="flex md:hidden items-center max-w-full space-x-1 sm:space-x-2 flex-shrink-0">
              ...
              <button 
                onClick={toggleDarkMode}
                className="p-1.5 sm:p-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-all duration-300"
              ...
              <button 
                ...
                className="p-1.5 sm:p-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-all duration-300 hover:shadow-md"
                aria-label="Menu"
              >
                {isOpen ? (
          ...
          {/* === MENU MOBILE EXPANDABLE - Responsive === */}
          {isOpen && (
            <div className="md:hidden border-t border-gray-200 dark:border-gray-700 pt-4 pb-4 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-xl sm:rounded-2xl mt-2 mx-1 sm:mx-2 shadow-xl">
              
              {/* Navigation mobile */}
              <div className="space-y-2 mb-6 px-3 sm:px-4">
                {navItems.map((item, index) => {
                  const isActive = activeSection === item.href.substring(1)
                  return (
                    <a
                      ...
                      href={item.href}
                      onClick={() => setIsOpen(false)}
                      className={`block px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base font-semibold transition-all duration-300 rounded-lg border-l-4 ${
                        isActive
                ...
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                  {professionalLinks.map((item, index) => (
                    <a
                      ...
                      rel="noopener noreferrer"
                      onClick={() => setIsOpen(false)}
                      className="flex items-center justify-center gap-2 p-2 sm:p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 transition-all duration-300 hover:shadow-md hover:bg-blue-600 hover:text-white hover:border-blue-600"
                      aria-label={item.label}
                    >
                      {item.icon}
